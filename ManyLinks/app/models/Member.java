 ...
 */
/*
 * Classe représentant les membres existants sur le site (Administrateur ou Utilisateur)
 */
@Entity
public class Member extends Model{
	...
	public boolean hasComment(String url){
		if (Link.findByURL(url) != null){
			if (Comment.count("byMemberIdAndLinkId", this.id,Link.findByURL(url).getId()) == 0){
				return false;
			}
			else
			{
	...
	public boolean hasCategory(String name){
		if (this.getPrivateCategoryList() != null){
			for (PrivateCategory privateCat : this.getPrivateCategoryList()){
				if (privateCat.getCategory().getName().equals(name)){
					return true;
				}
			}
			return false;
